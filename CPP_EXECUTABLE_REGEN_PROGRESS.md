...
- The retained executable-regeneration slice also moved this batch:
  ...
  - the retained mission-loader lane also moved in recovered C:
    ...
    - the menu-reachable `sub_460360` cases `0`, `1`, `2`, `3`, `4`, `5`, `6`, `10`, and `11` are now materialized from asm as campaign-entry map loaders (`k_mapa1l.map`, `k_mapa2l.map`, `k_mapa3l.map`, `k_mapa4j.map`, `k_mapa5j.map`, `k_mapa6j.map`, `k_mapa7z.map`, `p_mapa1z.map`, and `p_mapa2z.map`) with the corroborated player-state, name, unit, and castle setup
  ...
  - the surviving retained blocker is no longer the old `sub_40D330` / `sub_44C2A0` helper band, `mapK2`, `mapK3`, `mapK4`, `mapK5`, `mapK6`, or `mapK7`; it is the remaining `Scenario_LoadMissionByIndex` case recovery itself, starting with `mapK8` / case `7`
- The next executable-regeneration frontier remains split, not singular:
  ...
  - the next retained class/runtime target is no longer the slot/parser export band, the low-risk file/runtime wrapper band, the x87 math band, the earlier `PlayGame_Dispatch` UI/data/runtime band, the bare mission-loader `JUMPOUT`, the `sub_40D330` / `sub_44C2A0` helper gap, `mapK2`, `mapK3`, `mapK4`, `mapK5`, `mapK6`, or `mapK7`; it is the next remaining `Scenario_LoadMissionByIndex` case slice, starting with `mapK8`
