...
| `footprint_class_index` | high | Per-type footprint/overhang class used by rendering and redraw invalidation. | Rendering subtracts `byte_512577` from the sprite Y origin, world and battle redraw helpers use the same non-zero values to invalidate neighboring tiles, and `UnitBattle_GetFootprintClass` / `Unit_GetClassIndex` both expose the byte as a reusable per-type footprint-class lookup. | `clash95.c:23969-24017`, `clash95.c:24317-24320`, `clash95.c:27713-27722`, `clash95.c:40229-40243`, `clash95.c:44814-44875`, `clash95.c:45932-45960` | layout stat | large or overhanging sprites |
...
| `MixedStackCategory` | `road_move_cost`, `terrain_move_cost` | `UnitStack_BuildMergedTerrainMoveProfile` merges metadata bytes `+29..+37` by keeping the highest non-zero cost for each lane and collapsing to `0` when any member has `0`, so world-map movement follows the slowest traversable unit in the stack. | high | `clash95.c:26459-26517`, `clash95.c:26703-26748`, `clash95.c:27094-27134` |
...
- The battle sidebar labels `Defence power` and `Status` are confirmed by `off_514DC8`, but this pass still did not isolate the exact backing stat table/field pair behind those two rows.
- `footprint_class_index` is now the safest name for `byte_512577`, but the exact class semantics are still open: the value clearly affects draw height and footprint invalidation, yet it may still encode a broader visual-class enum rather than one strictly geometric footprint size.
