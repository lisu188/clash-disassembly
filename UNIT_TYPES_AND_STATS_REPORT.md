...
| `production_turns` | high | Per-type number of turns needed for a building to finish producing that unit. | `g_UnitTypeProductionTurns` seeds `BuildingRecord.production_turns_remaining`, and the building per-turn resolver decrements that countdown until the unit is spawned. | `clash95.c:32803-32833`, `clash95.c:54116-54127` | base stat | castle/settlement production |
| `production_cost` | high | Per-type local building-resource cost paid when production finishes. | `g_UnitTypeProductionCost` is checked against `BuildingRecord.stored_resource_pool` and then subtracted when the produced unit is inserted into the building garrison. | `clash95.c:32810-32831` | base stat | castle/settlement production |
