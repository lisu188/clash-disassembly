...
| `visual_class_index` | medium | Per-type visual/footprint class used by rendering and redraw invalidation. | Rendering subtracts `byte_512577` from the sprite Y origin, redraw helpers use the same non-zero values to invalidate neighboring tiles, and `Unit_GetClassIndex` proves the engine treats the byte as a reusable class lookup rather than a one-off raw offset. | `clash95.c:23969-24017`, `clash95.c:24317-24320`, `clash95.c:27713-27722`, `clash95.c:40229-40243`, `clash95.c:45932-45960` | layout stat | large or overhanging sprites |
...
- `visual_class_index` is now the safest name for `byte_512577`, but the exact class semantics are still open: the value clearly affects draw height and footprint invalidation, yet it may still encode a broader visual-class enum rather than a literal pixel offset.
