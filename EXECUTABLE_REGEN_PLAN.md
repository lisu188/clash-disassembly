...
- Take the next batch through:
  ...
  5. continue the retained mission-loader widening at the remaining case tranche starting with `mapK4` / case `3`, now that `mapK2` / case `1`, `mapK3` / case `2`, and their extra post-castle / camera-side effects are materialized in recovered C
...
- The retained `PlayGame_Dispatch` alias/data band is now reduced again:
  ...
  - the next retained widening should stay on the remaining mission-loader case arms, starting with `mapK4`, not on a new SDL seam or `src_cpp` abstraction
