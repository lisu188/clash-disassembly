...
int  UnitStack_GetVisionRadius(int stackPtr)
{
  ...
  maxRadius = (unsigned __int8)g_UnitTypeVisionRadius[UNIT_TYPE_METADATA_STRIDE * unitType];
  for ( slotIndex = 1; slotIndex < UNIT_STACK_SLOT_COUNT; ++slotIndex )
  {
    unitType = stack->unit_slots[slotIndex].unit_type_id;
    if ( unitType == -1 )
      break;
    slotRadius = (unsigned __int8)g_UnitTypeVisionRadius[UNIT_TYPE_METADATA_STRIDE * unitType];
