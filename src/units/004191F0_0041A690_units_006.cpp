...
#include "004191F0_0041A690_units_006_local.h"
...
unsigned __int16 * UI_DrawWidgetIcon(unsigned __int16 *result, int doRefresh)
{
  ...
  if ( sprite_set && sprite_index != -1 )
  {
    ...
    if ( doRefresh && g_RenderDevice != (_UNKNOWN *)&g_MainRenderDevice )
    {
      /*
        ...
        int destination_top)
{
  ...
  if ( !source )
    source = (_DWORD*)(&g_MainRenderDevice);
  if ( !destination )
    destination = (_DWORD*)(&g_MainRenderDevice);
...
int  UI_DrawWidgetIconWithTransition(unsigned __int16 *iconWidget, int animate, int transitionDuration, DWORD allocContext)
{
  ...
  if ( result != -1 )
  {
    ...
    if ( animate )
    {
      ...
      Render_FillRect((_DWORD*)(g_RenderDevice), (_DWORD *)(uintptr_t)savedSurface, iconWidget[2], *iconWidget, transSpriteHeight + *iconWidget - 1, SpriteWidth + iconWidget[2] - 1, 0, 0);
      prevRenderDevice = g_RenderDevice;
      g_RenderDevice = (_UNKNOWN*)(renderedSurface);
      ...
      g_RenderDevice = (_UNKNOWN*)(restoreRenderDevice);
      ...
      if ( g_RenderDevice != (_UNKNOWN *)&g_MainRenderDevice )
        result = Render_FillRect((_DWORD *)(uintptr_t)renderedSurfaceHandle, (_DWORD*)(g_RenderDevice), 0, 0, transBottom, transRight, *iconWidget, iconWidget[2]);
      if ( savedSurface )
...
BOOL  UIWidget_ShowTextWithFadeTransition(uintptr_t widget, DWORD allocContext)
{
  ...
  g_RenderDevice = (_UNKNOWN*)(textSurface);
        ...
        int drawContext)
{
  ...
  if ( spriteSet )
    spriteSet = DLXSpriteSet_Load(spriteSet, (const void*)(uintptr_t)((char)(intptr_t)slotRecordArg));
  v42 = spriteSet;
  ...
  if ( *((char *)slotRecord + 9) < 100 )
  {
    ...
    Render_FillRect(Surface, (_DWORD*)(g_RenderDevice), 5, 0, v19 - 1, healthBarFillRight, panelLeft + 42, panelTop + 5);
    if ( Surface )
        ...
        DWORD drawContext)
{
  ...
  if ( unitSpriteSet )
    unitSpriteSet = DLXSpriteSet_Load(unitSpriteSet, (const void*)(uintptr_t)(ownerIndex));
