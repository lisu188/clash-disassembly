...
#include "00460950_004612E0_strategic_006_local.h"
...
_DWORD * RenderState_LoadDefaultCursorSprite(int render_state)
{
  ...
  if ( result )
  {
    ...
    *(_DWORD *)(uintptr_t)(render_state + 64) = (_DWORD)(uintptr_t)(result);
  }
  else
  {
...
_DWORD * Device_GetParamA(int render_state, int a2)
{
  ...
  result[280] = (_DWORD)(uintptr_t)(g_Device_Vtable);
...
int  Device_UpdateRect(_DWORD *render_state, int a2, int a3)
{
  ...
  else
  {
    ...
    {
      /* 4613D1: EAX buffer, EDX element size, ECX stream, EBX count=1. */
      fread_(&g_DeviceRectTargetWidth, 2, replay_handle, 1);
      fread_(&g_DeviceRectTargetHeight, 2, tail->field_474_handle, 1);
      fread_(&g_DemoPlaybackCursorButtonState, 1, tail->field_474_handle, 1);
      v5 = 1;
      fread_(&g_DeviceReplaySegmentTargetTick, 4, tail->field_474_handle, 1);
    }
    trigger_count = 0;
    trigger_index = 0;
