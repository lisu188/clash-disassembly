...
int *WorldMap_RenderHook(DWORD first);
int *UnitBattle_InitBattleScreenFrame(void);
void *RenderHook_DemoText(int first, char second, DWORD context);
int *Castle_RebuildSceneBuffers(int first, DWORD context);
...
/* Clang emits C tentative definitions after functions. Keep that storage
 * order while C++ requires a single definition for each object. The legacy
 * audio descriptor placeholder must retain its original neighbours. */
extern unsigned __int16 __ES__;
extern unsigned __int16 __DS__;
extern __lock unk_51A638;
...
/* Shared with platform_sdl_runtime.cpp, which owns GetLastError/SetLastError. */
...
void __lock_p(__lock *this_)
{
  (void)this_;
}

void __lock_v(__lock *this_)
{
  (void)this_;
}
...
int Compat_WorldMapRenderHook(int first, char unused_char, DWORD unused_context)
{
  (void)unused_char;
  (void)unused_context;
  return (int)(intptr_t)WorldMap_RenderHook((DWORD)first);
}

int Compat_UnitBattleRenderHook(int unused_first, char unused_char, DWORD unused_context)
{
  (void)unused_first;
  (void)unused_char;
  (void)unused_context;
  return (int)(intptr_t)UnitBattle_InitBattleScreenFrame();
}

int Compat_DemoTextRenderHook(int first, char second, DWORD context)
{
  return (int)(intptr_t)RenderHook_DemoText(first, second, context);
}

int Compat_CastleRenderHook(int first, char second, DWORD unused_context)
{
  (void)unused_context;
  return (int)(intptr_t)Castle_RebuildSceneBuffers(first, (DWORD)second);
}
