...
signed int __fastcall Rules_PrintActivation(int logicalName, int activation)
{
  ...
  v9 = logicalName;
  sprintf_(((char*)(buffer)), "%-6d ", *(_DWORD *)(uintptr_t)(activation + 8));
...
signed int * Rules_SetSalienceEvaluationCommand(int context, double a2)
{
  ...
  if ( !strcmp_(*(_DWORD *)(uintptr_t)(v9[2] + 16), ((_DWORD)(uintptr_t)(aWhenDefined))) )
  {
    g_Rules_SalienceEvaluationMode = 0;
    v5 = currentModeName;
    return Str_Intern(v5, v4);
  }
  if ( !strcmp_(v4, ((_DWORD)(uintptr_t)(aWhenActivated))) )
  {
  ...
  {
    if ( !strcmp_(v7, ((_DWORD)(uintptr_t)(aEveryCycle))) )
...
static int Diagnostics_IsRuleFireTraceEnabled(void) CLASH95_LOCAL_SYMBOL(Diagnostics_IsRuleFireTraceEnabled);
static void Diagnostics_DumpAgenda(const char *tag) CLASH95_LOCAL_SYMBOL(Diagnostics_DumpAgenda);
...
{
  static int cached CLASH95_LOCAL_DATA_SYMBOL("cached.0", "Diagnostics_IsRuleFireTraceEnabled.cached") = -1;
  if ( cached < 0 )
  {
...
int  Rules_RunAgendaLoop(int runLimit, int a2, double a3)
{
  ...
  while ( activation )
  {
    if ( !remainingFires || g_ClipsHaltExecution || g_Rules_HaltRulesFlag )
      break;
    Rules_DetachActivation(((_DWORD*)(activation)));
    ...
    {
      a3 = sprintf_(((char*)(fireTraceBuffer)), "FIRE %4ld ", fireCount);
    ...
    *(_BYTE *)binds &= ~2u;
    ...
    Rules_RemoveActivation(((_DWORD*)(activation)), 0, 0);
  ...
  if ( g_Rules_WatchStatisticsFlag )
  {
    ...
    else
    {
      ...
      messageString = (char *)" rules per second.\n";
    }
    ...
    sprintf_(((char*)(statsBuffer)), "%ld mean number of facts (%ld maximum).\n", meanValue, maxFacts);
    ...
    sprintf_(((char*)(statsBuffer)), "%ld mean number of instances (%ld maximum).\n", meanValue, maxInstances);
    ...
    sprintf_(((char*)(statsBuffer)), "%ld mean number of activations (%ld maximum).\n", meanValue, maxActivations);
...
int Rules_NextActivationToFire(void)
{
  ...
  {
    mainModule = Module_FindByName(((_BYTE*)(aMain_0)));
...
signed int  Rules_BuildFocusStackList(_DWORD *returnValue)
{
  ...
  if ( g_Rules_FocusStackTop )
  {
    ...
    *(_DWORD *)(uintptr_t)(v5 + 8) = ((_DWORD)(uintptr_t)(multifield));
