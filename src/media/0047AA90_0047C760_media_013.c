  ...
  for ( i = 1; argExpr; ++i )
  {
    ...
    else if ( parsed[1] == 6 )
    {
      Rules_RetractFact(parsed[2], a3);   /* loc_47AEB2 */
           ...
           && !strcmp_(*(_DWORD *)((uintptr_t)(unsigned int)parsed[2] + 16), asc_502790) )
    {
...
signed int  Rules_FactsCommand(double frame)
{
  ...
  if ( !Module_FindByName((_BYTE *)(uintptr_t)*(_DWORD *)(uintptr_t)(theValue[2] + 16)) && strcmp_(moduleName, asc_502790) )
  {
    Lexer_ErrorRecover(1);
...
int  Rules_ParseSaveFactsCommand(double a1)
{
  ...
  if ( result )
  {
    if ( v5 > 1 )
    {
      ...
      if ( !result )
        return result;
      if ( strcmp_(*(_DWORD *)(uintptr_t)(v8[2] + 16), aLocal) )
      {
...
LABEL_11:
  ...
  if ( v20 != 2 )
    goto LABEL_11;
  deftemplate = Rules_FindImportExportConstruct(aDeftemplate_4, &tmpCnt, (_BYTE *)(uintptr_t)*(_DWORD *)(uintptr_t)(arrayEntry[2] + 16), 1, g_ClipsEvaluationError);
  if ( deftemplate )
    goto LABEL_11;
  v16 = exprIndex;
  *v19 = 1;
...
BOOL  Rules_LoadFactsFromFile(const CHAR *fileName, int a2, DWORD a3, double a4)
{
  ...
  do
  {
    ...
    if ( parsedForm )
      Parser_ParseForm(parsedForm, &v12, (int)(intptr_t)parsedForm, a4);
    else
      v13[0] = 102;
    AST_Free(v9);
  }
