...
_DWORD *__stdcall CSS_EmptySampleCache(void)
{
  ...
  ehFrame[2] = ((_DWORD)(uintptr_t)(&g_CSSEmptySampleCache_EHScopeTable));
  ehFrame[3] = 0;
...
_DWORD * CSS_CreateSampleCacheEntry(int stream, int fileName, _DWORD *formatInfo)
{
  ...
  CSS_Mem_TryAlloc(24, ((_DWORD*)(&entry)));
...
int  CSS_StartStreamVoice(int aviPlayer, int volume, int a3, DWORD panning)
{
  ...
  if ( bestChannel != -1 )
    CSS_CloseChannel(bestChannel, 0);
  *(_DWORD *)(uintptr_t)(g_SoundChannelArrayBase + 52 * bestChannel + 20) = ((_DWORD)(uintptr_t)(streamAdapter));
...
int __stdcall CSS_PlaySound(int fileName, int volume, int panning, signed int fadeMs)
{
  ...
  stream = Audio_ReadWavHeaderFromStream(fileName, ((_DWORD*)(formatInfo)));
  if ( !stream )
    return 0;
  if ( formatInfo[1] >= g_CSS_SampleCacheByteBudget )
    return CSS_StartSampleVoice(stream, formatInfo, panning, volume, fadeMs);
  CSS_PauseStreamReading();
  newEntry = (int *)CSS_CreateSampleCacheEntry(stream, fileName, (_DWORD *)formatInfo);
...
int __stdcall CSS_StopSound(int soundHandle, signed int fadeMs)
{
  ...
  ehFrame[0] = ((_DWORD)(uintptr_t)(NtCurrentTeb()->NtTib.ExceptionList));
  ehFrame[1] = ((_DWORD)(uintptr_t)(&j____wcpp_4_fs_handler_rtn_));
  ehFrame[2] = ((_DWORD)(uintptr_t)(&g_CSSStopSound_EHScopeTable));
  ehFrame[3] = 0;
  if ( soundHandle )
  {
    channelIndex = 0;
...
signed int  CSS_InitVoicePool(int voice_count, int stream_buffer_ms, int sample_cache_kb)
{
  ...
  CSS_Mem_TryAlloc(52 * voice_count, ((_DWORD*)(&g_SoundChannelArrayBase)));
  if ( voice_count > 0 )
  {
    channel_offset = 0;
    do
  ...
  hThread = (HANDLE)beginthreadex_(0, ((_DWORD)(uintptr_t)(thread_args)));
  g_CSS_StreamThreadHandle = (int)(uintptr_t)beginthreadex_(0, (_DWORD)(uintptr_t)thread_args);
...
_DWORD *CSS_ShutdownVoicePool(void)
{
  ...
  result = eh_frame;
  eh_frame[0] = ((_DWORD)(uintptr_t)(NtCurrentTeb()->NtTib.ExceptionList));
  eh_frame[1] = ((_DWORD)(uintptr_t)(&j____wcpp_4_fs_handler_rtn_));
  eh_frame[2] = ((_DWORD)(uintptr_t)(&g_CSSShutdownVoicePool_EHScopeTable));
  try_level = 0;
  if ( g_SoundChannelArrayBase )
  {
...
signed int __stdcall CSS_Init(int voice_count, int volume_scale, int stream_buffer_ms, int sample_cache_kb)
{
  ...
  else
  {
    ...
    thread_handle = (void *)beginthreadex_(0, ((_DWORD)(uintptr_t)(thread_args)));
