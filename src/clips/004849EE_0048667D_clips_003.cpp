...
/* This TU had four other local objects in the frozen C build. */
#define CLASH95_FAKE_TEB_GCC_SYMBOL "fake_teb.4"
#endif
...
/* Preserve the four function-local object names observed in the C17 objects. */
#if defined(__cplusplus) && defined(__clang__)
#define CLASH95_CLIPS_LOCAL_DATA_SYMBOL(gcc_name, clang_name) __asm__(clang_name)
#elif defined(__cplusplus) && defined(__GNUC__)
#define CLASH95_CLIPS_LOCAL_DATA_SYMBOL(gcc_name, clang_name) __asm__(gcc_name)
#else
#define CLASH95_CLIPS_LOCAL_DATA_SYMBOL(gcc_name, clang_name)
#endif
...
int CRT_ConstructBadExceptionFromCurrentSEH(void)
{
  ...
  *(_DWORD *)(uintptr_t)(result + 100) = (_DWORD)(uintptr_t)(&g_BadException_VTable);
  return result;
}
...
int __thiscall CRT_WriteMessageAndCountNewline(void *this_)
{
  ...
  result = fputs_((_DWORD)(uintptr_t)(this_), (_DWORD)(uintptr_t)(&g_CRT_StdoutStream));
  charsWritten = result;
  if ( result != -1 )
  {
    result = CRT_PutcToStream(v2, (_DWORD)(uintptr_t)(&g_CRT_StdoutStream));
    if ( result == 10 )
...
int  CRT_InitializeProcessStartupInfo(int threadDataMgmtDisabled, int a2, int a3, HMODULE moduleHandle)
{
  ...
  static const _WORD emptyWideCommandLine[] CLASH95_CLIPS_LOCAL_DATA_SYMBOL("emptyWideCommandLine.3", "CRT_InitializeProcessStartupInfo.emptyWideCommandLine") = { 0 };
...
int  CRT_InitializeRuntimeBeforeWinMain(int a1, int a2)
{
  ...
  g_CrtThreadDataAccessor(v6, (_DWORD)(uintptr_t)(&g_CRT_StartupThreadDataBlock));
  _init_stack_limits_();
...
signed int __fastcall Rules_PrintLongInteger(int logicalName, int number)
{
  ...
  static _BYTE printBuffer[32] CLASH95_CLIPS_LOCAL_DATA_SYMBOL("printBuffer.2", "Rules_PrintLongInteger.printBuffer");
  int v5; // [esp+20h] [ebp-4h]

  v5 = logicalName;
  sprintf_((char*)(printBuffer), "%ld", number);
...
unsigned int  Rules_PrintAtomValue(int logicalName, unsigned int type, int *value)
{
  ...
  static _BYTE addressBuffer[32] CLASH95_CLIPS_LOCAL_DATA_SYMBOL("addressBuffer.1", "Rules_PrintAtomValue.addressBuffer");
  ...
  if ( type < 8 )
  {
    if ( type == 5 )
    {
      ...
      sprintf_((char*)(addressBuffer), "%p", value);
...
int Rules_ExitCommand(void)
{
  ...
  if ( result != -1 )
    return IO_RunRouterExitCallbacks(-1);
  return result;
}
...
signed int  IO_AddRouter(int routerName, int priority, int printFunction, int queryFunction, int getcFunction, int ungetcFunction, int exitFunction)
{
  ...
  {
    new_router = (_DWORD *)(uintptr_t)Mem_HeapAllocWithRetry((_DWORD*)(intptr_t)(36));
  }
  new_router[1] = 1;
  new_router[8] = 0;
...
{
  static int reporting_router_error CLASH95_CLIPS_LOCAL_DATA_SYMBOL("reporting_router_error.0", "IO_ReportUnrecognizedRouter.reporting_router_error");
  int result; // eax
...
char *__thiscall CRT_AllocateLockSlot(void *this_)
{
  ...
  {
    allocatedSlot = calloc_((int)(intptr_t)(this_), 24);
    if ( !allocatedSlot )
      ((void)v3, (void)1, _fatal_runtime_error_());
  }
  else
  {
  ...
  if ( CurrentThreadId != *(_DWORD *)(uintptr_t)(lock + 8) )
  {
    if ( !*(_DWORD *)(uintptr_t)(lock + 4) )
    {
      ...
      if ( !*(_DWORD *)(uintptr_t)(lock + 4) )
      {
        ...
        *(_DWORD *)(uintptr_t)lock = (_DWORD)(uintptr_t)(criticalSection);
...
_BYTE *CRT_GetOrCreateThreadDataPreserveLastError(void)
{
  ...
  Value = (_BYTE*)(TlsGetValue(dwTlsIndex));
  v2 = Value;
  if ( !Value )
  {
...
int  CRT_CreateAndAttachThreadData(int a1, int a2)
{
  ...
  if ( result )
  {
    if ( !((void)v3, (void)result, _AddThreadData_()) )
    {
      nfree_(v5);
      return 0;
    }
