...
signed int  Rules_CheckFunctionArgCount(int functionName, _BYTE *restrictions, int argumentCount)
{
  ...
  if ( (IsTable[(unsigned __int8)(*restrictions + 1)] & 0x20) != 0 )
    minArguments = atoi_((const char*)(intptr_t)(functionName));
  else
    minArguments = -1;
  if ( (IsTable[(unsigned __int8)(restrictions[1] + 1)] & 0x20) != 0 )
    maxArguments = atoi_((const char*)(intptr_t)(v3));
  else
    maxArguments = 10000;
...
LABEL_10:
    ...
    *(_DWORD *)(uintptr_t)(curEntry + 16) = (_DWORD)(uintptr_t)(dataBuffer);
...
int  Rules_HashSymbolName(_BYTE *word, unsigned int range)
{
  ...
  if ( fullChunkCount )
  {
    do
    {
      ...
      chunkSrc = (char*)(&wordPtr[charIndex]);
  ...
  tailSrc = (char*)(&wordPtr[charIndex]);
  chunkValue = 0;
  if ( *tailSrc )
  {
...
int  Rules_DecrementSymbolCount(int result, int a2)
{
  ...
  if ( *(int *)(uintptr_t)(result + 4) < 0 )
  {
    ...
    result = IO_RunRouterExitCallbacks(5);
  }
...
LABEL_9:
  ...
  if ( anywhere )
  {
    if ( Rules_StringWithinString((_BYTE*)(symbolName), (_BYTE*)(searchString)) )
      return curSymbol;
    goto LABEL_9;
  }
  if ( prevSymbol )
  ...
  prefixLength = Rules_CommonPrefixLength((_BYTE*)(compareString), (_BYTE*)(curSymbol[4]));
