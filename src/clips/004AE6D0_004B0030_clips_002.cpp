...
signed int  MessageHandler_RemoveByNameAndType(_DWORD *theClass, int handlerName, int indicateMissing, int handlerType)
{
  ...
  if ( theClass[24] )
  {
    ...
    if ( handlerType == -1 )
    {
      ...
      if ( !found && !strcmp_(i, (_DWORD)(uintptr_t)(asc_5093F0)) )
      {
        i = theClass[24];
        scanIndex = 0;
        if ( i )
...
LABEL_19:
    ...
    {
      if ( !strcmp_(v15, (_DWORD)(uintptr_t)(asc_5093F0)) )
      {
        i = 0;
        if ( theClass[24] )
        {
...
signed int  Class_PurgeMarkedMessageHandlers(signed int result, unsigned int a2)
{
  ...
  if ( deletedCount )
  {
    ...
    else
    {
      ...
      *(_DWORD *)(uintptr_t)(theClass + 88) = (_DWORD)(uintptr_t)(newHandlerArray);
...
signed int  MessageHandler_TypeIndexFromKeyword(int typeName)
{
  ...
  typeIndex = 0;
  tableOffset = 0;
  while ( strcmp_(tableOffset, (_DWORD)(uintptr_t)(*(char **)((char *)g_MessageHandlerTypeNames + tableOffset))) )
  {
    tableOffset = v4 + 4;
    ++typeIndex;
    if ( tableOffset > 12 )
...
int  MessageHandler_BuildPreviewHandlerCore(int theClass, int messageName)
{
  ...
  return MessageHandler_JoinHandlerLinks(tops, (_DWORD*)(&bots[1]));
}
...
_DWORD * MessageHandler_CallNextHandler(_DWORD *returnValue, double a2)
{
  ...
  if ( !g_ClipsHaltExecution )
  {
    ...
    Symbol = Rules_MakeSymbol((_BYTE*)(aOverrideNext_0));
    if ( Symbol == *(int ***)(uintptr_t)(v5 + 2) )
    {
      ...
      result = (_DWORD *)(uintptr_t)(ProcParam_PushEvaluatedArgumentFrame((_DWORD*)(&exprType), argCount, v9, messageName, a2, (int (*)(void))Rules_ReportUndefinedMessageHandlerName));
