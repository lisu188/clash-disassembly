...
int  Lexer_BuildSlotNode(int theFact, char *slotName, _DWORD *returnValue)
{
  ...
  {
    if ( strcmp_((_DWORD)(uintptr_t)(slotName), (_DWORD)(uintptr_t)(aImplied)) )
    {
  ...
  else
  {
    ...
    if ( !Lexer_FindTemplateSlot(theDeftemplate, (int)(intptr_t)slotSymbol, (_DWORD*)(&slotPosition)) )
    {
      Lexer_ErrorRecover(1);
...
int  Rules_BuildFactSlotNameList(int theFact, _DWORD *returnValue)
{
  ...
  else
  {
    ...
    *(_DWORD *)(uintptr_t)(v8 + 8) = (_DWORD)(uintptr_t)(theList);
...
int Rules_BuildFactAddressList(void)
{
  ...
  v3[3] = 0;
  theList = Rules_CreateEphemeralMultifield(i);
  *(_DWORD *)(uintptr_t)(v7 + 8) = (_DWORD)(uintptr_t)(theList);
  if ( theModule )
  {
...
int  Rules_ResolveFactArgument(int argumentPosition, int theFunction, int noFactError, double a4)
{
  ...
  else if ( v8[1] == 1 && *(int *)(uintptr_t)(v8[2] + 16) >= 0 )
  {
    ...
    {
      sprintf_((char*)(tempBuffer), "f-%ld", factIndex);
    ...
    while ( 1 )
    {
      while ( 1 )
      {
        ...
        else
        {
          ...
          thePattern[4] = (_DWORD)(uintptr_t)(result);
        }
        if ( !thePattern[7] )
          break;
...
int  Rules_FactPatternNetworkBsaveStorage(int theFile)
{
  ...
  if ( result )
    return Rules_ConstructQueuePop((_DWORD*)(&g_FactPatternNodeCount));
  return result;
}
...
LABEL_6:
    ...
    while ( 1 )
    {
      ...
      ++itemCount;
      fileVersion = Rules_ConstructCodeFileClose(theFile, &itemCount, maxIndices, (_DWORD*)(&v14), 0, 0);
      thePattern = Rules_FactPatternNetworkNextNode(thePattern);
      if ( !thePattern )
        goto LABEL_6;
    }
...
int  Rules_AssignMultifieldRangeCopy(_DWORD *dst, int src)
{
  ...
  *(_DWORD *)(uintptr_t)(v5 + 8) = (_DWORD)(uintptr_t)(theCopy);
  result = 6 * fieldCount;
  qmemcpy(
