...
{
  return Rules_UndefconstructCommand((int)(intptr_t)this_, g_ClipsDefclassConstructType);
}
...
{
  return Rules_PPConstructCommand((int)(intptr_t)this_, (const char **)(uintptr_t)g_ClipsDefclassConstructType);
}
// 54E6B8: using guessed type int dword_54E6B8;
...
unsigned int * Class_InternSlotName(_DWORD *slotNameSymbol, int newSlotID, int useNewID)
{
  ...
  else
  {
    ...
    *(_DWORD *)(uintptr_t)(g_Defclass_SlotNameHashTablePtr + 4 * i) = (_DWORD)(uintptr_t)(newEntry);
    slotNameStr = (const char *)(uintptr_t)slotNameSymbol[4];
    ++slotNameSymbol[1];
    prefixCursor = aPut_0;
    temp_name_bytes = strlen(slotNameStr) + 5;
    destCursor = (_BYTE*)(Mem_SmallBlockAlloc(temp_name_bytes));
    putNameAlloc = (_DWORD*)(destCursor);
    putName = (char*)(destCursor);
    do
    {
      v15 = *prefixCursor;
