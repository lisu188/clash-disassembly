...
signed int  Rules_CheckExprTreeAgainstConstraint(__int16 *theExpression, int theConstraint)
{
  ...
  if ( theConstraint && theExpression )
  {
    do
    {
      ...
      if ( result )
        break;
      result = Rules_CheckExprTreeAgainstConstraint((short int*)(uintptr_t)(*(_DWORD *)(uintptr_t)(v5 + 6)), theConstraint);
      if ( result )
        break;
  ...
  while ( fieldWidth > 0 )
  {
    ...
    if ( (statusFlags & 1) != 0 )
    {
      if ( (statusFlags & 0x20) != 0 )
      {
        ...
        if ( Str_DecodeMultibyteChar((_WORD*)(&wideChar), mbBuffer, 2u) == -1 )
          return 0;
        *destBuffer++ = wideChar;
      }
      else
...
LABEL_42:
        if ( digitCount > 0 && (*(_BYTE *)(uintptr_t)(streamContext + 16) & 1) != 0 )
        {
          *bufferCursor = 0;
          ...
          {
            g_StringToDoubleScanFn(inputChar, (_DWORD)(uintptr_t)(doubleValue));
          }
          flagsByte = *(_BYTE *)(uintptr_t)(streamContext + 16);
          if ( (flagsByte & 4) != 0 )
          {
