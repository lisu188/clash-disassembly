  ...
  *result = item;
  ...
  if ( g_ClipsConstructQueueHead )
  {
    for ( result = (_DWORD *)(uintptr_t)g_ClipsConstructQueueHead; result[1]; result = (_DWORD *)(uintptr_t)result[1] )
      ;
    result[1] = (_DWORD)(uintptr_t)(newNode);
  }
  else
  {
...
LABEL_4:
    if ( *(_DWORD *)(i + 3) )
      LOWORD(result) = Rules_MarkReferencedFunctions((short int*)(uintptr_t)(*(_DWORD *)(i + 3)));
  }
...
const void * Rules_BsaveWriteEndTag(int a1, int a2)
{
  ...
  footerBuffer[6] = a2;
  strncpy_(a1, (_DWORD)(uintptr_t)(g_Rules_BloadFileHeaderID));
...
unsigned int  AST_HashNodeChildren(_DWORD *theConstraint)
{
  ...
  if ( *(_DWORD *)((char *)theConstraint + 26) )
    i += AST_HashNodeChildren((_DWORD*)(uintptr_t)(*(_DWORD *)((char *)theConstraint + 26)));
  return i % 0xA7;
}
...
LABEL_5:
      ...
      *(_DWORD *)(uintptr_t)(g_ConstraintHashTable + 4 * savedHash) = (_DWORD)(uintptr_t)(theConstraint);
...
int  Rules_SetDynamicConstraintCheckingCommand(int returnValue, double context)
{
  ...
  if ( argCheck == -1 )
    return v4;
  Rules_RtnUnknown(1, (_DWORD*)(&argBuffer), context);
...
{
  return Rules_UndefconstructCommand((int)(intptr_t)this_, g_Clips_DefgenericConstructType);
}
// 54E6A0: using guessed type int dword_54E6A0;
...
{
  return Rules_GetConstructModuleCommand((int)(intptr_t)this_, (const char **)(uintptr_t)g_Clips_DefgenericConstructType);
}
// 54E6A0: using guessed type int dword_54E6A0;
...
signed int  Defgeneric_UndefmethodCommand(int returnValue, double context)
{
  ...
  if ( result )
  {
    if ( Symbol_LookupInModule((char **)(uintptr_t)g_Clips_DefgenericConstructType, (_BYTE *)(uintptr_t)*(_DWORD *)(uintptr_t)(argData[2] + 16), 1) || !strcmp_(0, (_DWORD)(uintptr_t)(asc_50529C)) )
    {
      Rules_RtnUnknown(2, argData, context);
      if ( argData[1] == 2 )
      {
        methodIndex = strcmp_(theGeneric, (_DWORD)(uintptr_t)(asc_50529C));
        if ( methodIndex )
        {
LABEL_7:
...
_DWORD * Defgeneric_FormatMethodRestrictionsString(const char *buffer, int bufferMax, int methodIndex, int theGeneric)
{
  ...
  return Defgeneric_AppendMethodSignatureText((char *)(buffer), bufferMax, v7, 40 * methodPosition + *(_DWORD *)(uintptr_t)(theGeneric + 28));
}
...
{
  return Rules_PPConstructCommand((int)(intptr_t)this_, (const char **)(uintptr_t)g_Clips_DefgenericConstructType);
}
// 54E6A0: using guessed type int dword_54E6A0;
