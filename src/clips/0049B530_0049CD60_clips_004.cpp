...
const CHAR * Rules_DribbleOnCommand(DWORD a1, double context)
{
  ...
  if ( Lexer_TokenExpect((int)(intptr_t)aDribbleOn, 0, 1) == -1 )
    return 0;
...
{
  if ( Lexer_TokenExpect((int)(intptr_t)aDribbleOff, 0, 0) == -1 )
    return 0;
  else
...
{
  return strcmp_((_DWORD)(uintptr_t)(this_), (_DWORD)(uintptr_t)(aStdin_2)) == 0;
}
...
int  Rules_BatchReadChar(int logicalName, unsigned int returnOnEOF)
{
  ...
  do
  {
    ...
    else
    {
      ...
      if ( remainingChars > 0 && (returnOnEOF = *(unsigned __int8 *)(uintptr_t)*(_DWORD *)(uintptr_t)g_CurrentStringInputSource - 13, returnOnEOF > 0xFD) )
      {
...
{
  Rules_CloseAllBatchSources(this_);
  return 1;
}
...
const CHAR * Rules_BatchCommand(DWORD a1, double context)
{
  ...
  if ( Lexer_TokenExpect((int)(intptr_t)aBatch, 0, 1) == -1 )
    return 0;
  result = (const CHAR *)(uintptr_t)Rules_GetFileNameArg(1, v2, context);
  if ( result )
...
_DWORD * Rules_AddBatchEntry(int placeAtEnd, int theSource, int theString, int sourceType)
{
  ...
  freeListHead = (_DWORD *)(uintptr_t)*(_DWORD *)(uintptr_t)(g_ClipsMemoryTable + 64);
  if ( freeListHead )
  {
  ...
  *result = sourceType;
  ...
  if ( g_Rules_BatchStackTop )
  {
    if ( placeAtEnd )
    {
      *(_DWORD *)(uintptr_t)(g_Batch_ListTail + 12) = (_DWORD)(uintptr_t)(result);
...
signed int Rules_CloseBatch(void)
{
  ...
  if ( *(_DWORD *)(uintptr_t)g_Rules_BatchStackTop )
  {
    ...
    Mem_SmallBlockFree((_DWORD *)(uintptr_t)*(_DWORD *)(uintptr_t)(g_Rules_BatchStackTop + 8), strlen((const char *)(uintptr_t)*(_DWORD *)(uintptr_t)(g_Rules_BatchStackTop + 8)) + 1);
  }
  else
  {
    fclose_(0);
...
signed int __thiscall Rules_CloseAllBatchSources(void *this_)
{
  ...
  {
    if ( g_BatchEchoBufferLength > 0 )
      Output_Write((int)(intptr_t)aStdout_2, g_ClipsBatchEchoLineBuffer, (int)(intptr_t)this_);
    Mem_SmallBlockFree((_DWORD *)(uintptr_t)g_ClipsBatchEchoLineBuffer, g_ClipsBatchLineBufferCapacity);
    g_ClipsBatchEchoLineBuffer = 0;
    g_BatchEchoBufferLength = 0;
    g_ClipsBatchLineBufferCapacity = 0;
...
const CHAR * Rules_BatchStarCommand(DWORD a1, double context)
{
  ...
  if ( Lexer_TokenExpect((int)(intptr_t)aBatch_0, 0, 1) == -1 )
    return 0;
  result = (const CHAR *)(uintptr_t)Rules_GetFileNameArg(1, v2, context);
  if ( result )
...
int  Rules_LoadCommand(DWORD a1, double context)
{
  ...
  if ( Lexer_TokenExpect((int)(intptr_t)aLoad, 0, 1) == -1 )
    return 0;
...
const CHAR * Rules_LoadStarCommand(DWORD a1, double context)
{
  ...
  if ( Lexer_TokenExpect((int)(intptr_t)aLoad_0, 0, 1) == -1 )
    return 0;
...
int  Rules_SaveCommand(DWORD a1, double context)
{
  ...
  if ( Lexer_TokenExpect((int)(intptr_t)aSave, 0, 1) == -1 )
    return 0;
  result = Rules_GetFileNameArg(1, v2, context);
  fileName = result;
  if ( result )
...
int  Rules_LoadConstructsFromLogicalName(int readSource, int a2)
{
  ...
  if ( Rules_SkipToConstructBeginning(readSource, (_DWORD*)(&theToken), v5, 0) == 1 )
  {
    do
    {
    ...
    while ( Rules_SkipToConstructBeginning(readSource, (_DWORD*)(&theToken), v8, errorCorrection) == 1 );
  }
        ...
        int moduleNameAllowed)
{
  ...
  moduleSeparator = Rules_FindModuleSeparator((_BYTE *)(uintptr_t)*(_DWORD *)(uintptr_t)(*(_DWORD *)(uintptr_t)(inputToken + 4) + 16));
  ...
  if ( moduleSeparator )
  {
    ...
    constructNameSymbol = Rules_ExtractConstructName(separatorPosition, (char *)(uintptr_t)*(_DWORD *)(uintptr_t)(v18 + 16), v18);
...
LABEL_23:
  ...
  else
  {
    ...
    if ( moduleNameAllowed )
    {
      ...
      IO_OutWriteToken((char *)(uintptr_t)*(_DWORD *)(uintptr_t)(v36 + 16));
    }
  }
...
signed int Rules_RegisterControlFlowFunctions(void)
{
  ...
  Rules_SetFunctionSeqOverloadFlags((_BYTE*)(aProgn), 0);
  Rules_SetFunctionSeqOverloadFlags((_BYTE*)(aIf), 0);
  Rules_SetFunctionSeqOverloadFlags((_BYTE*)(aWhile), 0);
  Rules_SetFunctionSeqOverloadFlags((_BYTE*)(aLoopForCount), 0);
  Rules_SetFunctionSeqOverloadFlags((_BYTE*)(aReturn), 0);
  return Rules_SetFunctionSeqOverloadFlags((_BYTE*)(aSwitch), 0);
}
...
int  Rules_WhileFunction(double context)
{
  ...
  int result; // eax
  _DWORD theResult[6]; // [esp+0h] [ebp-30h] BYREF
  /* stack alias of theResult[1] */
  /* stack alias of theResult[2]: the DATA_OBJECT value slot */
  /* stack alias of theResult[3] */
  /* stack alias of theResult[4] */
  ...
  if ( g_ClipsHaltExecutionFlag == 1 )
  {
    returnValue[1] = theResult[1];
    returnValue[2] = theResult[2];
    returnValue[3] = theResult[3];
    result = theResult[4];
    returnValue[4] = theResult[4];
  }
  else
  {
    returnValue[1] = 2;
...
int  Rules_IfFunction(_DWORD *returnValue, double context)
{
  ...
  /* sub_49CD60: `mov ecx, eax` - eax is the DATA_OBJECT the caller wants filled
     (Parser_ParseForm passes valuePtr). IDA dropped the parameter entirely and
     left the undefined temps v2/v4 in its place. */
  result = Rules_ArgRangeCheck((int)(intptr_t)aIf, 2, 3);
  argCount = result;
  if ( result == -1 )
  {
    returnValue[1] = 2;
  ...
  else
  {
    ...
    if ( g_ClipsBreakFlag != 1 && g_ClipsHaltExecutionFlag != 1 )
    {
      if ( g_ClipsFalseSymbol == returnValue[2] && returnValue[1] == 2 && argCount == 3 )
      {
        return Rules_RtnUnknown(3, returnValue, context);
      }
      else if ( g_ClipsFalseSymbol == returnValue[2] && returnValue[1] == 2 )
      {
        returnValue[2] = g_ClipsFalseSymbol;
      }
      else
      {
        return Rules_RtnUnknown(2, returnValue, context);
      }
    }
  }
  return result;
