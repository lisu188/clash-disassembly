...
LABEL_20:
    *(_DWORD *)(uintptr_t)(returnValue + 4) = 1;
    result = (int*)(intptr_t)(Rules_AddIntegerValue(parseStatus));
  ...
  if ( item2[1] == 1 && !*(_DWORD *)(uintptr_t)(item2[2] + 16) || item2[1] != 1 && *(double *)(uintptr_t)(item2[2] + 16) == 0.0 )
  {
    ...
    Lexer_ErrorRecover(1);
    *(_DWORD *)(uintptr_t)(returnValue + 4) = 1;
    result = (int*)(intptr_t)(Rules_AddIntegerValue(0));
    *(_DWORD *)(uintptr_t)(returnValue + 8) = (_DWORD)(uintptr_t)(result);
  }
  else if ( item1[1] && item2[1] )
  {
    ...
    *(_DWORD *)(uintptr_t)(returnValue + 4) = 1;
    result = (int*)(intptr_t)(Rules_AddIntegerValue(intRemainder));
    *(_DWORD *)(uintptr_t)(returnValue + 8) = (_DWORD)(uintptr_t)(result);
  }
  else
  {
...
_DWORD * Help_IndexTopicFile(CHAR *fileName, _BYTE *errorBuffer, int a3, int errorBufferSize, DWORD a5)
{
  ...
  if ( fp )
  {
    ...
    if ( fileRecord )
    {
      ...
      inEntry = 0;
      entryClosed = 1;
      while ( fgets_((char*)(lineBuffer), 256, (int)(intptr_t)fp) )
      {
        ++lineNumber;
        if ( lineBuffer[0] != 36 || lineBuffer[1] != 36 )
        {
          if ( Str_FindSubstringIndex(lineBuffer, (_BYTE*)(aEndEntry)) < 0 )
          {
            if ( Str_FindSubstringIndex(lineBuffer, (_BYTE*)(aBeginEntry)) >= 0 )
            {
              if ( entryClosed != 1 )
              {
          ...
          else
          {
            if ( inEntry != 1 )
            {
              ...
              if ( v11 < 60 )
                return 0;
              sprintf_((char*)(errBuf), "Line %d : Unmatched end marker.", lineNumber);
              return 0;
            }
            entryClosed = 1;
            inEntry = 0;
      ...
      {
LABEL_23:
        sprintf_((char*)(errBuf), "Line %d : Previous entry not closed.", lineNumber);
        return 0;
      }
    }
    else
    {
      ...
      if ( v10 >= 60 )
      {
        sprintf_((char*)(errBuf), "File \"%s\" already loaded.", fileName);
        return 0;
      }
    }
  }
  ...
  {
    sprintf_((char*)(errBuf), "Could not open file \"%s\".", fileName);
  }
  return 0;
}
...
int  Help_ReopenFileAtActiveEntry(const CHAR *fileName, _DWORD *code, DWORD a3)
{
  ...
  if ( g_HelpLoadedTopicFileList )
    noMatch = strcmp_(g_HelpLoadedTopicFileList, (_DWORD)(uintptr_t)(fileName)) != 0;
  else
    noMatch = 0;
  if ( noMatch > 0 )
  {
    ...
    while ( fileRecord && strcmp_(fileRecord, (_DWORD)(uintptr_t)(fileName)) );
  }
  if ( !fileRecord )
  {
    *code = -10;
...
_BYTE * Help_ReadEntryLine(int stream_handle, _BYTE *buffer, int buffer_size)
{
  if ( fgets_((char *)buffer, buffer_size, stream_handle) )
  {
    ...
    if ( Str_FindSubstringIndex(buffer, (_BYTE*)(aEndEntry)) < 0 )
      return buffer;
  }
...
LABEL_6:
    ...
    *((_DWORD *)newRecord + 22) = 0;
    if ( g_HelpLoadedTopicFileList )
    {
      *(_DWORD *)(uintptr_t)(v6 + 88) = (_DWORD)(uintptr_t)(newRecord);
      return newRecord;
    }
    else
    {
  ...
  {
LABEL_5:
    result = (char *)(uintptr_t)strcmp_(record, (_DWORD)(uintptr_t)(fileName));
    if ( !result )
      return result;
    goto LABEL_6;
  }
...
_DWORD * Help_ParseEntryHeader(int *fp, int fileName, int errorBuffer, signed int errorBufferSize, int lineNumber)
{
  ...
  if ( sscanf_((const char*)(intptr_t)(v9), "%d%1s%12s%s", entry, &entryTypeChar, &delimiterString, entry + 2) == 4 )
  {
    if ( entryTypeChar == 77 )
    {
      v10[1] = -45;
LABEL_5:
      if ( !strcmp_((_DWORD)(uintptr_t)(v10), (_DWORD)(uintptr_t)(aBeginEntry)) )
      {
        bufferedCount = fp[1];
        ...
        {
          nextChar = fgetc_(v11, (_DWORD)(uintptr_t)(fp));
        }
      ...
      else
      {
        ...
        if ( errorBufferSize >= 60 )
          sprintf_((char*)(intptr_t)(errorBuffer), "Line %d : Invalid delimeter string.", lineNumber);
        return 0;
      }
    }
    if ( entryTypeChar == 73 )
    ...
    Help_UnloadTopicFile(fileName);
    if ( v23 >= 60 )
      sprintf_((char*)(intptr_t)(errorBuffer), "Line %d : Invalid entry type.", lineNumber);
    return 0;
  }
  else
  {
