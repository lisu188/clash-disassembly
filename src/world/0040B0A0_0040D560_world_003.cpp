...
#include "0040B0A0_0040D560_world_003_local.h"
        ...
        double st0_runtime)
{
  ...
  int (*saved_render_hook)(int, char, DWORD);
...
int  PlayGame(int a1, char a2, DWORD allocContext, char a4, double st0_runtime, ...)
{
  ...
  *(_DWORD *)(uintptr_t)(gameData + MAP_VIEW_TOP_OFFSET) = *(_DWORD *)(uintptr_t)(PLAYER_DATA_STRIDE * *(_DWORD *)(uintptr_t)(gameData + VIEWED_PLAYER_INDEX_OFFSET) + gameData + PLAYER_CAMERA_TOP_TABLE_OFFSET);
  ...
  g_RenderHook = Compat_WorldMapRenderHook;
...
void  TextSprite_BuildOrLoadCachedFont(int slotIndex, _BYTE *fontData, int a3, char a4, DWORD allocContext)
{
  ...
  if ( cacheFileExists )
  {
    ...
    if ( cachedSpriteSet )
      cachedSpriteSet = DLXSpriteSet_Load(cachedSpriteSet, (const void*)(uintptr_t)((char)(intptr_t)checksum));
  ...
  else
  {
    ...
    if ( spriteSet )
      loadedSpriteSet = DLXSpriteSet_Load(spriteSet, (const void*)(uintptr_t)((char)(intptr_t)checksum));
...
int Font_InitGlyphFallbackTablesForLanguage()
{
  ...
  Font_ResetGlyphFallbackTable((_WORD *)&g_TerrainAnimationRemapTable);
  Font_ResetGlyphFallbackTable((_WORD *)&g_SpriteCodeRemapTable);
