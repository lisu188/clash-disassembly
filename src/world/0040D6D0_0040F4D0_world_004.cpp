...
int  MiniMap_RedrawTileRect(int x0, signed int y0, int y1, int x1)
{
  ...
  if ( PLAYER_MINIMAP_VISIBLE(VIEWED_PLAYER_INDEX) )
  {
    ...
    return Render_FillRect((_DWORD*)(g_RenderDevice), 0, boxTop, boxLeft, boxRight, boxBottom, boxLeft, boxTop);
  }
  return result;
}
...
void * MiniMap_DrawTileCell(void *result, signed int tileColumn)
{
  ...
  g_RenderDevice = (_UNKNOWN*)(previous_render_device);
  return result;
}
// 511230: using guessed type _UNKNOWN *g_RenderDevice;
...
LABEL_52:
      ...
      if ( hitIndex != -1 )
      {
        ...
        if ( v22 )
        {
          ...
          {
            v63 = UI_RunMenu((_WORD*)(uintptr_t)(*(_DWORD *)(hitEntry + 15)), prevHoveredIndex);
            if ( v63 )
              hoveredIndex = -1;
          }
        }
...
BOOL  WorldMap_HandleTopMenuBar(char a1, int a2)
{
  ...
  int (*previous_render_hook)(int, char, DWORD); // [esp+2Ch] [ebp-18h]
  const WorldMapTopMenuHitRecord *top_menu_record;
