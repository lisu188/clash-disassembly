...
/* Legacy CRT/Win32-era import declarations without manifest definitions (282); typed against compatibility/decomp_runtime_stubs.cpp or explicit native call contracts. */
...
int __cdecl CSyncObject_Unlock(CSyncObject *this_, __int32, __int32 *);
...
BOOL __stdcall DuplicateHandle(HANDLE hSourceProcessHandle, HANDLE hSourceHandle, HANDLE hTargetProcessHandle, HANDLE *lpTargetHandle, DWORD dwDesiredAccess, BOOL bInheritHandle, DWORD dwOptions);
...
int __stdcall ICGetDisplayFormat(int codec, _DWORD input_format, _DWORD output_format, int depth, int width, int height);
int __stdcall ICLocate(int type, _DWORD handler, _DWORD input_format, _DWORD output_format, int mode);
...
__int64 __fastcall _I8M(_DWORD low, _DWORD high);
...
int _allocfp_(void);
int __fastcall _brktime_(_DWORD value, _DWORD context);
int __fastcall _ccmdline_(int first, int second);
int _chktty_(void);
int __fastcall _clib_ulltoa_(_DWORD a1, _DWORD a2);
int __fastcall _doclose_(int stream, int close_mode);
...
_DWORD __cdecl bad_exception_dtor(bad_exception *this_);
HANDLE __stdcall beginthreadex_(_DWORD a1, _DWORD a2);
int __fastcall calloc_(int count, int size);
...
int __fastcall fread_(void *buffer, int size, int file_handle, int count);
...
int __fastcall memcpy_(int destination, int source);
int __thiscall nfree_(_DWORD); // weak
int __fastcall nrealloc_(int first, int second);
...
int __fastcall wcslen_(int first, const void *second);
int __fastcall wcstombs_(_DWORD destination, _DWORD source);
