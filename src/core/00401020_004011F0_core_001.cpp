...
int App_Shutdown(void)
{
  ...
  return Render_BeginModeSwitch((_DWORD*)(&g_MainRenderDevice));
}
...
int  Game_Init(int a1, char a2, DWORD allocContext)
{
  ...
  if ( result )
  {
    result = _wcpp_4_ctor_array__(result + UNIT_STACK_TABLE_OFFSET, UNIT_STACK_TABLE_COUNT) - UNIT_STACK_TABLE_OFFSET;
    gameData = result;
  }
  else
  {
...
char __thiscall DetectGameCDPath(void *this_)
{
  ...
  *(_DWORD *)&pathBuffer[44] = (_DWORD)(uintptr_t)this_;
  driveLetter = 67;
