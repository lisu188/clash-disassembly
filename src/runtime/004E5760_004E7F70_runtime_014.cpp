...
int  AST_IsFieldPositionConsistent(_DWORD *theNode, int whichPattern)
{
  ...
  {
    result = AST_IsFieldPositionConsistent(((_DWORD*)(uintptr_t)(currentNode[17])), whichPattern);
    if ( !result )
      return result;
...
void  CRT_PutEnvEntryA(_BYTE *envString, _BYTE *i, int a3)
{
  ...
  if ( envString )
  {
    ...
    if ( *i )
    {
      ...
      if ( i[1] )
      {
        newTable = (_DWORD *)nmalloc_(a3, ((_DWORD)(uintptr_t)(i)));
...
LABEL_22:
          *(_DWORD *)(uintptr_t)(envTable + 4 * entryIndex) = ((_DWORD)(uintptr_t)(envString));
...
signed int  CRT_PutEnvEntryW(_WORD *envString, _WORD *i)
{
  ...
  {
    allocatedTable = (_DWORD *)nmalloc_(0, ((_DWORD)(uintptr_t)(i)));
...
LABEL_22:
      *(_DWORD *)(uintptr_t)(envTablePtr + 4 * entryIndex) = ((_DWORD)(uintptr_t)(envString));
...
const CHAR * CRT_SetEnvironmentVariableCompat(const WCHAR *wideName, const WCHAR *wideValue, int a3)
{
  ...
  if ( result )
  {
    ...
    if ( wcstombs_(v7, (_DWORD)(uintptr_t)wideName) == -1 )
    {
      nfree_(v12);
      if ( !ansiValue )
        return 0;
LABEL_9:
    ...
    if ( ansiValue && wcstombs_(v12, (_DWORD)(uintptr_t)wideValue) == -1 )
      goto LABEL_9;
