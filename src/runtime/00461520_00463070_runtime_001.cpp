...
LRESULT __thiscall Platform_MainWindowProc(void *this_, HWND hWnd, UINT Msg, WPARAM wParam, LPARAM lParam)
{
  ...
  Debug_Log((int)(intptr_t)this_, Msg, lostStatus, (int)(intptr_t)aDmessage0x08xL);
  if ( Msg < 0xF )
  {
    if ( Msg == 2 )
    {
...
WPARAM  Platform_PumpMessagesAndBlitFrame(char a1)
{
  ...
  Render_BlitSurface(((_DWORD*)(&g_MainRenderDevice)), 0, a1, (DWORD)(intptr_t)savedregs);
  while ( 1 )
  {
...
void  Video_Avi_playIn(const char *aviName, int a2, int fadeOutFirst, int a4, int switchTo16bpp, int switchBackTo8bpp)
{
  ...
  int v13; // ecx
  int (*renderHook)(int, char, DWORD); // ebp
  int v15; // ecx
  int v16; // ecx
  int v17 CLASH95_UNUSED; // ecx
  char pathBuffer[100]; // [esp+0h] [ebp-80h] BYREF
  int (*savedRenderHook)(int, char, DWORD); // [esp+64h] [ebp-1Ch]
  ...
  {
    Render_BlitSurface(((_DWORD*)(&g_MainRenderDevice)), 1, a4, (DWORD)(intptr_t)aviName);
  }
...
signed int  Win_PlayModeChangeFrameTransition(const char *aviName, int restorePalette, int a3, char a4, DWORD a5, ...)
{
  ...
  int (*savedRenderHook)(int, char, DWORD); // [esp+90h] [ebp-14h]
...
{
  if ( !strcmp_(aviName, ((_DWORD)(uintptr_t)(aKon_por1_1))) )
  {
...
{
  *result = ((_DWORD)(uintptr_t)(g_IOStreamAdapter_Vtable));
  result[1] = aviPlayer;
  return result;
}
...
_DWORD * IO_StreamAdapterRelease(_DWORD *adapter, char freeFlags)
{
  ...
  if ( (freeFlags & 4) != 0 )
  {
    _wcpp_4_dtor_array_store__(((_DWORD)(uintptr_t)(adapter)), ((_DWORD)(uintptr_t)(&g_IOStreamAdapter_DtorArrayTag)));
    j_j__nfree_();
    return (_DWORD *)(uintptr_t)v5;
  }
  else
  {
    *adapter = ((_DWORD)(uintptr_t)(g_IOStreamAdapter_Vtable));
    if ( (freeFlags & 2) != 0 )
      j__nfree_();
    return v3;
  }
...
int  ExcString_ConstructEmpty(_DWORD *excStr)
{
  ...
  *excStr = 0;
  excStr[1] = ((_DWORD)(uintptr_t)(&g_CompatStringHolder_Vtable));
