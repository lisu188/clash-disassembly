...
//----- (0041D030) --------------------------------------------------------
BOOL  Building_New(int buildingType, DWORD stackIndex, double st7_0, char *name, int force)
{
...
char  Building_GetInto(int buildingIndex, char a2, DWORD a3)
{
  ...
  int (*previousRenderHook)(int, char, DWORD); // ebx
  ...
  if ( !*(_WORD *)(uintptr_t)(buildingPtr + 16) )
  {
    if ( !*(_BYTE *)(uintptr_t)(buildingPtr + 4) )
    {
      ...
      g_RenderHook = Compat_DemoTextRenderHook;
