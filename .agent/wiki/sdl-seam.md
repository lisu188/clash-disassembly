...
- Current evidence:
  ...
  - the old retained `sub_40D330` / `sub_44C2A0` gap plus the next retained `mapK2` and `mapK3` cases were resolved entirely in recovered C without touching `platform_sdl_runtime.c`
- Next likely SDL-facing work is still later input/present fidelity, not the current mission-loader or class/bload blocker; the next retained blocker is `mapK4`, still below SDL.
