...
- Current frontier:
  ...
  - the retained mission-loader slice in `clash95.c` now covers cases `0`, `1`, `2`, `3`, `4`, `10`, and `11`
...
- Highest runtime milestone:
  ...
  - `mapK5` / case `4` is now materialized in recovered C with its exact `Totaweon` / `Hopenberg` setup, raw garrison/prisoner writes, and `Rules_LogAssignedCastleFact(..., 4)` tail preserved
...
- Adjacent blocked widening:
  ...
  - the next retained widening is the remaining `Scenario_LoadMissionByIndex` case tranche, starting with `mapK6`
- Core notes:
