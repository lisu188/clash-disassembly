...
- Current frontier:
  ...
  - the retained mission-loader slice in `clash95.c` now covers cases `0`, `1`, `2`, `3`, `4`, `5`, `10`, and `11`
...
- Highest runtime milestone:
  ...
  - `mapK6` / case `5` is now materialized in recovered C with its localized name split, player-runtime side effects, `Defambrion` / `Katha Gha` / `Ghih Up` building-field writes, raw 10-slot status loop, and `Rules_LogAssignedPlayerFact(3, 5)` tail preserved
...
- Adjacent blocked widening:
  ...
  - the next retained widening is the remaining `Scenario_LoadMissionByIndex` case tranche, starting with `mapK7`
- Core notes:
