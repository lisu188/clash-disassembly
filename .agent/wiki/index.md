...
- Current frontier:
  ...
  - the retained mission-loader slice in `clash95.c` now covers cases `0` through `13`
...
- Highest runtime milestone:
  ...
  - case `13` / `p_mapa4l.map` is now materialized in recovered C with its `Raylin` / `Leryks X` / `Glazur` / `Sir John` setup, the single `Ungught` castle lane, the random per-slot `Rng_RandRange(5, 20)` loop at `TILE_INDEX(25, 28)`, and the preserved absence of any post-castle building writes or post-init camera override
...
- Adjacent blocked widening:
  ...
  - the next retained widening is the remaining `Scenario_LoadMissionByIndex` case tranche, starting with case `14` / `p_mapa5l.map`
- Core notes:
