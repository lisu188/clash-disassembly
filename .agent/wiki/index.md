...
- Current frontier:
  ...
  - the retained mission-loader slice in `clash95.c` now covers cases `0` through `11`
...
- Highest runtime milestone:
  ...
  - `mapK10` / case `9` is now materialized in recovered C with its five-player `Alan` / `Twogor` / `Drebegen` / `Mieszko` / `Chester` setup, the seven-castle lane, the case-local `BUILDING_RECORD(+438)` cuts/assignments/addition, the direct masked `BUILDING_RECORD(+444)` writes, and the preserved absence of any raw stack loops or post-init camera override because asm does too
...
- Adjacent blocked widening:
  ...
  - the next retained widening is the remaining `Scenario_LoadMissionByIndex` case tranche, starting with `mapP3`
- Core notes:
