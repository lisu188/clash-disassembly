...
- Current frontier notes:
  ...
  - cases through `17` / `p_mapa8j.map` are confirmed and recovered as pure recovered-C mission setup; the next honest class/runtime widening is still not a signal to move behavior into `src_cpp`, and the live retained blocker is now the remaining mission-case recovery starting with case `18` / `p_mapa9j.map`
