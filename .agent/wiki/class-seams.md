...
- Current frontier notes:
  ...
  - the narrowed retained mission-loader blocker is no longer the bare `Scenario_LoadMissionByIndex` / `sub_460360` `JUMPOUT`: the first menu-reachable cases are now recovered in `clash95.c`, and the latest class-seam audit still says the remaining helper band `sub_40D330` / `sub_44C2A0` is pure recovered-C mission setup rather than `src_cpp` work
  - the latest retained misses are still not a signal to move behavior into `src_cpp`; they remain local recovered-C/data work until the surviving class/runtime evidence says otherwise
