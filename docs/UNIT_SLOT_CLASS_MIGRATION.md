...
After extraction commit `33d32b6`, the four exact definitions move into
`src/units/UnitSlot.cpp`. Both their complete definition text and canonical
body hashes are checked against extraction. The original TU keeps its adapters,
storage and callback identities. The new TU is appended to the manifest-backed
source inventory: **146 TUs, 17 methods in six class-named files**.

The same actual-source test, both production builds, strict current-main surface
comparisons and all eight asset-free gates pass again (`unit-slot-relocation`
logs). Coverage metadata resolves the same two frozen identities to their new
canonical source; neither the 718 set nor the numerical floor changes. The
header generator's unchanged file timestamps are preserved in this local
validation to avoid needless recompilation; generated bytes remain verified.
