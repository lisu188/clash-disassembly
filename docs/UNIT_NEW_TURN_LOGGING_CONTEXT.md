...
repair before UnitTurn class extraction. Date: 2026-09-06. This change does not
advance a campaign endpoint or visual-fidelity claim.
...
Unit_NewTurn, LogAllUnits and Debug_Log definitions through the source manifest
and verifies their hashes. It compiles them in separate translation units.
...
Scratch root: `/var/tmp/clash95-class-unit-turn-20260906/`.
The `recovery/` directory holds the separate source patch, body-hash metadata,
production-resolving test and fixture. `evidence/recovery-production-first/`
contains the four passing profiles on the coordinating agent's repaired source;
`evidence/recovery-true-negative-control/` contains all four expected failures of
the unrepaired control. `evidence/recovery-relocated-class/` repeats the same
recovery test after scratch class extraction/relocation. Every directory has
commands, generated source and compiler/run logs in its summary JSON or files.
The older `repair-contract-first-matrix.json` records an initial fixture-only
missing-include compile failure; `repair-contract-matrix.json` records its
corrected four-profile result. Failed attempts remain preserved.
