...
This file is the working guide for contributors recovering `clash95` from the
original binary artifacts and the current `clash-disassembly` repo.
...
- **repo-confirmed**: a rename or struct already exists in
  `docs/archive/REVERSE_ENGINEERING_RENAME_LOG.md` or
  `RECOVERED_STRUCTURES.json`.
