...
alignas(16) static unsigned char state[600000];
...
static void check_records(std::initializer_list<signed char> keys,
                          std::initializer_list<int> expected, int count, int keep) {
    assert(keys.size() == expected.size() && (size_t)count <= keys.size());
    check_records(keys.begin(), expected.begin(), count, keep);
}
int main(void) {
    check_scores();
    check_records({9, 1, 8, 2}, {1, 2, 3, 0}, 4, 2);
    check_records({1, 2, 0}, {0, 1, 2}, 3, 0);
    check_records({-1, 1}, {0, 1}, 2, 0);
    check_records({1, -1}, {1, 0}, 2, 0);
    check_records({8}, {0}, 1, 1);
    check_records({8, 8}, {0, 1}, 2, 2);
    check_records({0}, {0}, 0, 0);
    signed char keys[22];
    int expected[22];
...
class NativeStrategicDataflowTests(unittest.TestCase):
    def test_production_bodies_with_both_optimization_and_char_modes(self):
        ...
        compilers = [next((shutil.which(name) for name in family if shutil.which(name)), None)
                     for family in (("g++-13", "g++"), ("clang++-18", "clang++"))]
        compilers = [compiler for compiler in compilers if compiler is not None]
        if not compilers:
            self.skipTest("native C++ compiler unavailable")
        source = (ROOT / "src/strategic/004589C0_004602F0_strategic_004.cpp").read_text()
            ...
            for compiler in compilers:
                for optimize in ("-O0", "-O2"):
                    for char_mode in ("-fsigned-char", "-funsigned-char"):
                        with self.subTest(compiler=compiler, optimize=optimize, char_mode=char_mode):
                            binary = Path(directory) / "strategic"
                            command = [compiler, "-std=gnu++20", "-U_GNU_SOURCE", "-fno-exceptions", "-fno-rtti", optimize, char_mode, "-Wall", "-Wextra", "-Werror", "-Wno-parentheses", "-fsanitize=undefined,float-cast-overflow", "-fno-sanitize-recover=all", "-fsanitize-undefined-trap-on-error", str(path), "-o", str(binary)]
