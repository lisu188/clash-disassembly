#!/usr/bin/env python3
"""Actual methods, adapters and frozen repaired functions against original low32 cases."""
from pathlib import Path
import hashlib,importlib.util,json,os,platform,shutil,subprocess,sys,tempfile,unittest
sys.dont_write_bytecode=True
ROOT=Path(os.environ.get('CLASH95_SOURCE_ROOT',Path(__file__).resolve().parents[2])).resolve();FIXTURE=ROOT/'tests/tools/fixtures/spent_turn_repair';sys.path.insert(0,str(ROOT/'tools'))
...
class SpentTurnRepairTests(unittest.TestCase):
 ...
 def test_actual_bodies_original_pointer_and_complete_bytes(self):
  ...
  repaired=json.loads((FIXTURE/'repaired-references/provenance.json').read_text())
  for item in repaired['functions']:
   name=item['name'];self.assertEqual(rows[name]['implementation']['kind'],'method');p=FIXTURE/'repaired-references'/item['fixture'];t=p.read_text();d,=scan_definitions(t,{name})
   self.assertEqual(hashlib.sha256(p.read_bytes()).hexdigest(),item['file_sha256']);self.assertEqual(body_sha256(t,d),item['body_sha256']);self.assertEqual(rows[name]['legacy_body_sha256'],item['legacy_body_sha256'])
   bodies.append(t.replace(name+'(', 'Reference_'+name+'(',1))
  ...
  with tempfile.TemporaryDirectory(prefix='clash95-spent-turn-') as temporary:
   ...
   finally:(out/'summary.json').write_text(json.dumps({'case_count':streams['case_count'],'lanes':['frozen-repaired','adapter','direct-method'],'native_pointer_width':8,'profiles':results},indent=2)+'\n')
