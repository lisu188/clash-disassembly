 ...
 for mode in (0,):
  ...
  for prefix in range(11):
   for delta in (-2,-1,0,1,2):
    for flags in (0,1,2,3,127,128,254,255):yield row(mode,0x80000000-31*prefix+delta,prefix,flags)
def expected(data):
 ...
 for i in range(10):
  ...
  image[51+31*i]|=8
  processed+=1
 result=(base+31*processed)&0xffffffff
 return result.to_bytes(4,'little')+image
