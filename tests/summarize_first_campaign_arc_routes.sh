...
route_dir="${CLASH95_FIRST_CAMPAIGN_ARC_ROUTE_DIR:-$repo_root/tests/first_campaign_arc_routes}"
missions="${CLASH95_FIRST_CAMPAIGN_ARC_MISSIONS:-00 01 02 03 04 05 06 07 08 09}"
require_complete="${CLASH95_FIRST_CAMPAIGN_ROUTE_SUMMARY_REQUIRE_COMPLETE:-0}"
...
if [ "$require_complete" = "1" ]; then
  if [ "$partial_count" -ne 0 ] || [ "$incomplete_count" -ne 0 ] || [ "$missing_count" -ne 0 ] || [ "$unknown_count" -ne 0 ]; then
    echo "first-campaign route summary is not all-complete" >&2
    exit 1
  fi
fi
