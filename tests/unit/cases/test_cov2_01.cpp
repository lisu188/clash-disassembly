...
/* ------------------------------------------------------------------ */
/* Fog overlay results measured from original 0x40F0C0 instruction bytes. */
/* Exercise actual visibility memory and style effects for every mask.   */
/* ------------------------------------------------------------------ */
static void cov2_01_set_revealed(unsigned char *gd, int player, int row, int col) {
  ...
  gd[byte_off] |= (unsigned char)(1u << (col & 7));
}

TEST(cov2_01_fogofwar, populated_gamedata_sweep) {
  /* Bits: NW80 N40 NE20 W10 E08 SW04 S02 SE01. The original table
   * covers every single direction, opposite/adjacent pair and larger mask. */
  static const signed char expected[256] = {
  ...
  int saved = gameData;
  char saved_style = g_TextSprite_StyleFlag;
  CHECK((uintptr_t)gd <= UINT32_MAX);
  memset(gd, 0, sizeof gd);
  *(_DWORD *)(gd + 140000) = 100;
  *(_DWORD *)(gd + 140004) = 100;
  gameData = (int)(intptr_t)gd;

  for (int player = 0; player < 6; ++player) {
    for (unsigned int mask = 0; mask < 256; ++mask) {
      memset(gd + 140024, 0, 1423 * 6);
      for (int bit = 0; bit < 8; ++bit)
        if (mask & (1u << bit))
          cov2_01_set_revealed(gd, player, 50 + dx[bit], 50 + dy[bit]);
      g_TextSprite_StyleFlag = (char)0xa5;
      CHECK_EQ(Map_ClassifyFogOfWarOverlayForPlayer(50, 50, player), expected[mask]);
      CHECK_EQ((unsigned char)g_TextSprite_StyleFlag, mask ? 1 : 0xa5);

      /* A different player must still see a completely hidden region. */
      g_TextSprite_StyleFlag = (char)0xa5;
      CHECK_EQ(Map_ClassifyFogOfWarOverlayForPlayer(50, 50, (player + 1) % 6), 0);
      CHECK_EQ((unsigned char)g_TextSprite_StyleFlag, 0xa5);

      /* Revealing the center short-circuits every neighbor combination. */
      cov2_01_set_revealed(gd, player, 50, 50);
      g_TextSprite_StyleFlag = (char)0xa5;
      CHECK_EQ(Map_ClassifyFogOfWarOverlayForPlayer(50, 50, player), -1);
      CHECK_EQ((unsigned char)g_TextSprite_StyleFlag, 0xa5);
    }
  }
  gameData = saved;
  g_TextSprite_StyleFlag = saved_style;
}
