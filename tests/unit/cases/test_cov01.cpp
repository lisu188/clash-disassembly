...
TEST(cov01_unitslot, init_from_type) {
  ...
  memset(buf, 0, sizeof buf);
  TOUCH(UnitSlot_InitFromType((int)(intptr_t)buf, (unit_type)0, 0));

  /* unitType == -1 -> both action-points/morale assignments are skipped */
  memset(buf, 0, sizeof buf);
  TOUCH(UnitSlot_InitFromType((int)(intptr_t)buf, (unit_type)-1, 0));
  ...
  buf[22] = (char)0xFF;
  buf[13] = (char)0xFF;
  TOUCH(UnitSlot_InitFromType((int)(intptr_t)buf, (unit_type)0, 3));
  ...
  memset(buf, 0, sizeof buf);
  TOUCH(UnitSlot_InitFromType((int)(intptr_t)buf, (unit_type)18, 0));
}
...
TEST(cov01_unitstats, icon_indices_use_slot_stance_not_tail) {
  alignas(__int16) unsigned char bytes[sizeof(UnitSlotRecord)] = {0};
