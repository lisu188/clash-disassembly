...
## Batch 43 – Connection Overlay Shape Wave
| Old Name / Pattern | New Name | Kind | Subsystem | Confidence | Evidence Summary | Sources |
|---|---|---|---|---|---|---|
| sub_423BB0 | ConnectionOverlay_HasNorthLink | Function | Map / Tile Connections | High | Reads the northern neighbor's tile word at offset `+4`, canonicalizes special attachment variants through `ConnectionOverlay_GetCanonicalShapeId`, and accepts only the shapes that expose a south-facing link back into the current tile. | c, asm |
| sub_423C50 | ConnectionOverlay_HasSouthLink | Function | Map / Tile Connections | High | Symmetric south-neighbor predicate: canonicalizes the southern neighbor's connection overlay and accepts only the shapes that expose a north-facing link back into the current tile. | c, asm |
| sub_423CF0 | ConnectionOverlay_HasWestLink | Function | Map / Tile Connections | Medium | Checks whether the western side of the current tile is connectable either through the western neighbor's canonical overlay shape or through a special adjacent building footprint occupying the two tiles immediately to the west. | c, asm |
| sub_423E10 | ConnectionOverlay_HasEastLink | Function | Map / Tile Connections | High | Symmetric east-neighbor predicate: canonicalizes the eastern neighbor's connection overlay and accepts only the shapes that expose a west-facing link back into the current tile. | c, asm |
| sub_423E90 | ConnectionOverlay_RebuildTileShape | Function | Map / Tile Connections | High | Builds a four-bit west/south/east/north neighbor mask from the directional link predicates and uses `g_ConnectionOverlayShapeByNeighborMask` to rewrite the current tile's connection overlay word, or clear it to `0xFFFF` when no links remain. | c, asm |
| sub_423FC0 | ConnectionOverlay_GetCanonicalShapeId | Function | Map / Tile Connections | High | Collapses special attachment/transition overlay ids into their canonical connection-shape ids before connectivity tests, mapping the `877..949` family back to base shapes `866` and `867` and routing the `819..861` family through the older indirection table. | c, asm |
| dword_5141A0 | g_ConnectionOverlayShapeByNeighborMask | Table | Map / Tile Connections | High | Lookup table used only by `ConnectionOverlay_RebuildTileShape`; its low 16 entries map four-neighbor link masks directly to the base connection-shape ids `866..876`. | c, asm |
| MapTileRecord + 4 `road_or_bridge_tile_id` | MapTileRecord + 4 `connection_overlay_tile_id` | Field | Map / Tile Connections | Medium | The same tile word stores ordinary road/bridge connection art plus special attachment variants that are normalized by `ConnectionOverlay_GetCanonicalShapeId`, so the broader `connection_overlay` name matches the implementation better than a road-only label. | c, asm |

## Batch 44 – Road Build Placement Loop Wave
| Old Name / Pattern | New Name | Kind | Subsystem | Confidence | Evidence Summary | Sources |
|---|---|---|---|---|---|---|
| sub_425120 | RoadBuild_UpdateAdjacentPreview | Function | Map / Road Build UI | High | Verifies whether the hovered tile is one of the four adjacent build targets, updates the matching directional widget descriptor's screen position with a small animation offset, and marks the preview active only when the current tile can accept a road or special connection build. | c, asm |
| sub_4254E0 | RoadBuild_ActivateDirectionWidget | Function | Map / Road Build UI | High | Decodes the clicked widget's action id `0x1B..0x1E` into directions `0/2/4/6`, temporarily suppresses the overlay draw hook, commits `Road_Build` in that direction, then restores the preview hook. | c, asm |
| sub_425540 | RoadBuild_RunPlacementLoop | Function | Map / Road Build UI | High | Enters the modal road-placement loop for the selected stack, swaps in the road-build cursor/theme, animates the four directional widgets around the stack, pumps input/rendering, and exits when a build succeeds, the cursor leaves previewable tiles, or an explicit exit request is raised. | c, asm |
| dword_514294 | g_RoadBuildPreviewPixelOffsetByAnimFrame | Table | Map / Road Build UI | High | Nine-entry oscillation table used only by `RoadBuild_UpdateAdjacentPreview` to nudge the directional widget positions while previewing adjacent road targets. | c, asm |
| dword_527C30 | g_RoadBuildLoopExitRequested | Global | Map / Road Build UI | High | Modal loop latch for `RoadBuild_RunPlacementLoop`: initialized to zero on entry, set by the small exit helpers, and checked each frame to terminate road-placement mode. | c, asm |
| dword_527C34 | g_RoadBuildPreviewActive | Global | Map / Road Build UI | Medium | Cleared before each redraw during road placement and set by `RoadBuild_UpdateAdjacentPreview` only when the current hovered adjacent tile yields a valid preview target. | c, asm |
| dword_527C38 | g_RoadBuildPreviewAnimFrame | Global | Map / Road Build UI | High | Cycled `0..7` every 10 ticks during `RoadBuild_RunPlacementLoop` and consumed through `g_RoadBuildPreviewPixelOffsetByAnimFrame` to animate the four directional road-build widgets. | c, asm |
| dword_5142B8 / dword_5142ED / dword_514322 / dword_514357 region | RoadBuildDirectionWidgetDescriptor | Recovered Struct | Map / Road Build UI | Medium | Asm exposes four contiguous 53-byte static records with shared draw and activate procs; the road-build preview helper rewrites their first two dwords as live screen coordinates and indexes them by direction. | c, asm |

## Batch 45 – Build Placement Overlay And Widget State Wave
| Old Name / Pattern | New Name | Kind | Subsystem | Confidence | Evidence Summary | Sources |
|---|---|---|---|---|---|---|
| dword_52698C | g_MapTileOverlayDrawHook | Global | Map / UI / Placement | High | The world-tile renderer calls this callback after `UI_DrawTileHighlightOverlay`, road-build mode installs `RoadBuild_UpdateAdjacentPreview` there, and building placement installs `BuildPlacement_DrawTileOverlay`, proving it is a generic per-tile overlay draw hook rather than a road-only global. | c, asm |
| sub_4250F0 | RoadBuild_AnimateExitWidgetPress | Function | Map / Road Build UI | High | Asm shows the helper loading `edx = 1`, calling the generic widget press animation helper, then storing that value into `g_RoadBuildLoopExitRequested`, so it is the animated road-build exit-button handler. | c, asm |
| sub_425110 | RoadBuild_RequestExit | Function | Map / Road Build UI | High | Pure helper that sets `g_RoadBuildLoopExitRequested = 1`; used as the inactive/default activate proc in the road-build modal control records. | c, asm |
| sub_429E90 | BuildPlacement_AnimateExitWidgetPress | Function | Map / Build Placement UI | High | Same pattern as the road-build exit helper: animates the pressed widget then stores `1` into the build-placement loop exit latch. | c, asm |
| sub_429EB0 | BuildPlacement_RequestExit | Function | Map / Build Placement UI | High | Pure setter for the build-placement modal exit latch, used as the default activate proc in the static build-action widget records. | c, asm |
| sub_429EC0 | BuildPlacement_DrawTileOverlay | Function | Map / Build Placement UI | High | Installed into `g_MapTileOverlayDrawHook` during `BuildBuilding`; per rendered tile it validates the current placement footprint against `sub_41DBA0` and draws either the tinted valid footprint or the blocked-placement sprite over the candidate tile(s). | c, asm |
| dword_531CD0 | g_BuildPlacementLoopExitRequested | Global | Map / Build Placement UI | High | Central modal-loop latch for `BuildBuilding`: initialized to zero on entry, set by the two placement-exit helpers, and polled each frame until the loop terminates. | c, asm |
| dword_531CD4 | g_BuildPlacementTypeId | Global | Map / Build Placement UI | Medium | Stores the `a1` build-kind selector passed into `BuildBuilding` and is then consumed by `BuildPlacement_DrawTileOverlay` plus the footprint validator to distinguish 1x1 trap/road-style placement from 2x2 building placement. | c, asm |
| qword_531CD8 | g_BuildPlacementOriginTile | Global | Map / Build Placement UI | High | Holds the current candidate placement tile under the cursor as a packed row/column pair; `BuildBuilding` updates it on mouse movement, repaints old/new footprint tiles, and passes it directly into `Trap_New` and `Building_New`. | c, asm |
| unk_5146C0 region | BuildPlacementActionWidgetDescriptor | Recovered Struct | Map / Build Placement UI | Medium | Asm exposes a second 53-byte static widget family rooted at `unk_5146C0` with localized action labels (`Road`, `Trap`, `Castle`, `Keep`, `Fortress`, `Dig for treasure`), shared draw proc `sub_4191F0`, and activate procs that `BuildBuilding` swaps between default exit and animated-exit handlers. | c, asm |

## Batch 46 – Building Transfer Dialog State Wave
| Old Name / Pattern | New Name | Kind | Subsystem | Confidence | Evidence Summary | Sources |
|---|---|---|---|---|---|---|
| sub_42A990 | BuildingTransferDialog_RequestCloseAnimated | Function | Building / Transfer UI | High | Animates the pressed widget and writes the resulting nonzero state into the dialog exit latch; the modal transfer loop waits on that latch before tearing the dialog down. | c, asm |
| sub_42A9B0 | BuildingTransferDialog_ConfirmTransfer | Function | Building / Transfer UI | High | Confirms the current transfer dialog, applies pending peasant transfer through `Building_Transfer(..., mode 0, ...)`, applies pending gold transfer through `Building_Transfer(..., mode 1, ...)`, then redraws both numeric transfer bands. | c, asm |
| sub_42AD70 | BuildingTransferDialog_DecreasePendingPeasants | Function | Building / Transfer UI | High | Decreases the pending peasant transfer slider by 10-step increments, clamps it at zero, and redraws the top transfer band. | c, asm |
| sub_42AE30 | BuildingTransferDialog_IncreasePendingPeasants | Function | Building / Transfer UI | High | Increases the pending peasant transfer slider in 10-step increments, clamps it against the source building's peasant population at offset `+430`, and redraws the top transfer band. | c, asm |
| sub_42AF10 | BuildingTransferDialog_DecreasePendingGold | Function | Building / Transfer UI | High | Decreases the pending gold transfer slider by 10-step increments, clamps it at zero, and redraws the bottom transfer band. | c, asm |
| sub_42AFD0 | BuildingTransferDialog_IncreasePendingGold | Function | Building / Transfer UI | High | Increases the pending gold transfer slider in 10-step increments, clamps it against the source building's gold pool at offset `+438`, and redraws the bottom transfer band. | c, asm |
| sub_42B0A0 | BuildingTransferDialog_Run | Function | Building / Transfer UI | High | Initializes the modal castle transfer dialog for one source building, loads the themed sprite set, seeds the widget family and slider state, pumps the dialog until the exit latch changes, then frees the sprite set and restores rendering state. | c, asm |
| dword_531CE4 | g_BuildingTransferDialogSpriteSet | Global | Building / Transfer UI | High | Sprite-set handle loaded at dialog startup and shared by the transfer dialog widget descriptors. | c, asm |
| dword_531CE8 | g_BuildingTransferDialogExitRequested | Global | Building / Transfer UI | High | Modal exit latch polled by `BuildingTransferDialog_Run` and written by the animated close helper. | c, asm |
| dword_531CEC | g_BuildingTransferPendingPeasants | Global | Building / Transfer UI | High | Pending transfer amount for mode `0`; `Building_Transfer` mode `0` exports UnitType32 peasant cargo and subtracts this amount from building word `+430`, proving the peasant semantics. | c, asm |
| dword_531CF0 | g_BuildingTransferPendingGold | Global | Building / Transfer UI | High | Pending transfer amount for mode `1`; `Building_Transfer` mode `1` exports UnitType31 gold cargo and subtracts this amount from building dword `+438`, proving the gold semantics. | c, asm |
| dword_531CF4 | g_BuildingTransferSourceBuildingPtr | Global | Building / Transfer UI | High | Pointer to the source building record currently shown in the transfer dialog; every slider clamp and confirm path dereferences this record directly. | c, asm |
| BuildingRecord + 430 | BuildingRecord.peasant_population_count | Field | Building / Economy | High | `Building_Transfer` mode `0` subtracts from this low-12-bit word and emits UnitType32 peasant cargo, while the settlement economy helpers grow and shrink the same field each turn. | c |
| BuildingRecord + 438 | BuildingRecord.stored_gold | Field | Building / Economy | High | `Building_Transfer` mode `1`, add-on purchases, wall upgrades, and per-turn settlement income all consume or produce this dword, and UnitType31 gold cargo is created from it during transfer. | c |
| dword_514840 region | BuildingTransferDialogWidgetDescriptor | Recovered Struct | Building / Transfer UI | Medium | Asm exposes a contiguous 53-byte widget family rooted at `dword_514840` whose handlers match the transfer-dialog plus/minus controls, close button, and confirm-transfer button, all sharing `g_BuildingTransferDialogSpriteSet`. | c, asm |

## Batch 47 – Settlement Economy Struct Wave
| Old Name / Pattern | New Name | Kind | Subsystem | Confidence | Evidence Summary | Sources |
|---|---|---|---|---|---|---|
| sub_43EE50 | Building_GetTaxPressureTier | Function | Building / Economy | High | Reads peasant population from building `+430`, picks one of three population bands, then compares the low 6 bits of building byte `+436` against the four-entry threshold slice in `g_TaxPressureThresholdsByPopulationBand`, returning a tier `0..3` that feeds the growth algorithm. | c, asm |
| sub_43EED0 | Building_UpdatePopulationGrowth | Function | Building / Economy | High | Uses the previous signed growth delta at `+432` to adjust settlement condition, computes a new growth percentage from tax pressure, add-ons, tech level, and condition, then stores the signed population-growth delta back at `+432` and adds it into population `+430`. | c, asm |
| sub_43F0C0 | Building_CollectGoldIncome | Function | Building / Economy | High | Converts peasant population and the low 6 bits of byte `+436` into a gold income amount, suppresses income during active crises at `+435`, stores the per-turn amount at `+442`, and adds it into `stored_gold` at `+438`. | c, asm |
| sub_43F160 | Building_UpdateSettlementCrisis | Function | Building / Economy | Medium | If the low 3 bits of byte `+435` are nonzero, decrements that timer and removes 15-25% of the peasant population; otherwise, very large settlements can randomly arm the same timer. The exact original crisis label remains unclear, but the timer/state behavior is secure. | c, asm |
| sub_42AB80 | BuildingTransferDialog_DecreaseTaxRate | Function | Building / Transfer UI | Medium | Decrements the low 6-bit tax-rate setting at building byte `+436`, animates the notice box redraw, and is bound to the first transfer-dialog minus button whose widget descriptor carries the `kasa` token. | c, asm |
| sub_42AC80 | BuildingTransferDialog_IncreaseTaxRate | Function | Building / Transfer UI | Medium | Increments the low 6-bit tax-rate setting at building byte `+436` up to `40`, animates the notice box redraw, and is bound to the paired `kasa` plus button in the transfer dialog widget family. | c, asm |
| byte_515D00 | g_TaxPressureThresholdsByPopulationBand | Table | Building / Economy | High | Twelve meaningful threshold bytes arranged as three 4-entry slices for small/medium/large population bands; used only by `Building_GetTaxPressureTier` to map tax settings into growth-penalty tiers. | c, asm |
| BuildingRecord + 432 | BuildingRecord.population_growth_delta | Field | Building / Economy | High | `Building_UpdatePopulationGrowth` treats this word as a signed 12-bit growth delta, first folding its previous value back into condition `+434`, then overwriting it with the newly computed population growth before adding that amount into `+430`. | c |
| BuildingRecord + 434 | BuildingRecord.settlement_condition_percent | Field | Building / Economy | Medium | 0-100 meter initialized to `50`, nudged by prior growth, and used to penalize or improve both income and population growth. The behavior is secure even though the original designer-facing label could have been morale, approval, or prosperity. | c |
| BuildingRecord + 435 | BuildingRecord.settlement_crisis_state | Field | Building / Economy | Medium | Low 3 bits are a crisis countdown that suppresses income and shrinks population while active; `Building_UnitGetInto` can also arm it when a special flagged stack enters the building. The exact crisis label remains ambiguous, so the name stays generic. | c |
| BuildingRecord + 436 | BuildingRecord.tax_rate_setting | Field | Building / Economy | Medium | Low 6 bits store the adjustable settlement tax slider. Higher values improve `Building_CollectGoldIncome` directly but worsen the tier returned by `Building_GetTaxPressureTier`, reducing long-term growth. | c |
| BuildingRecord + 442 | BuildingRecord.gold_income_last_turn | Field | Building / Economy | High | Written only by `Building_CollectGoldIncome` immediately before the same amount is added into `stored_gold`, making it a clean cached last-turn income field. | c |

## Batch 48 – Building Transfer Destination List State Wave
| Old Name / Pattern | New Name | Kind | Subsystem | Confidence | Evidence Summary | Sources |
|---|---|---|---|---|---|---|
| sub_42A760 | UI_DrawNoticeBoxSmall | Function | UI / Building Transfer | High | `clash95.map` preserves the original symbol at `0x42A760`, and the body draws the small transfer status panel that reflects `population_growth_delta` and `settlement_crisis_state` through sprite ids `3/4/5`. | c, map, asm |
| sub_42A890 | UI_DrawConfirmTop | Function | UI / Building Transfer | High | `clash95.map` preserves the original symbol at `0x42A890`, and the body renders the top numeric confirmation box for pending peasant transfer. | c, map, asm |
| sub_42A910 | UI_DrawConfirmBottom | Function | UI / Building Transfer | High | `clash95.map` preserves the original symbol at `0x42A910`, and the body renders the lower numeric confirmation box for pending gold transfer. | c, map, asm |
| sub_436620 | BuildingTransferDialog_DrawDestinationList | Function | UI / Building Transfer | High | Lazily loads the castle-list sprite set, draws the 5-row scrolling destination list, highlights the current selection, and renders either the `-2` sentinel label or the destination building name and icons. | c |
| sub_4368F0 | BuildingTransferDialog_SelectPreviousDestination | Function | UI / Building Transfer | High | Decrements the current destination index when possible, redraws the list, and is wired as the left-scroll control for the destination chooser. | c, asm |
| sub_436930 | BuildingTransferDialog_SelectNextDestination | Function | UI / Building Transfer | High | Advances only when the one-slot lookahead entry is valid, then redraws the chooser. The decompiler-emitted `word_532362` is just an alias over the same list buffer. | c, asm |
| sub_436970 | BuildingTransferDialog_BuildDestinationList | Function | UI / Building Transfer | High | Clears the destination list buffer, seeds entry `0` with sentinel `-2`, appends eligible owned building indices, and resets the current selection index before the dialog opens. | c |
| sub_436A30 | BuildingTransferDialog_HandleDestinationListClick | Function | UI / Building Transfer | High | Converts the clicked row into a relative `-2..2` offset, validates that the visible slot resolves to a real list entry, updates the selection index, and redraws the list. | c |
| sub_436AF0 | BuildingTransferDialog_FreeDestinationListSpriteSet | Function | UI / Building Transfer | High | Exact teardown pair for the lazily-loaded destination-list sprite set allocated in `BuildingTransferDialog_DrawDestinationList`. | c |
| dword_53235C | g_BuildingTransferDestinationSelectionIndex | Global | UI / Building Transfer | High | Shared current-selection index used by the chooser draw path, left/right handlers, click handler, and transfer confirm logic. | c |
| word_532360 | g_BuildingTransferDestinationBuildingIds | Global | UI / Building Transfer | High | Contiguous `int16` list of destination building ids. Entry `0` is initialized to sentinel `-2`, and later entries are populated by `BuildingTransferDialog_BuildDestinationList`. | c |
| word_532362 | g_BuildingTransferDestinationBuildingIdsLookahead | Global Alias | UI / Building Transfer | High | Not a separate array in the binary: this decompiler symbol aliases `g_BuildingTransferDestinationBuildingIds + 1` and is only used as a one-slot lookahead in the “next destination” predicate. | c |
...
- `g_BuildPlacementTypeId` is secure as the modal build-kind selector used by `BuildBuilding` and `BuildPlacement_DrawTileOverlay`, but the full numeric enum behind its values still needs a focused pass beyond the currently corroborated trap/building footprint split.
- `BuildingTransferDialogWidgetDescriptor` is secure as one transfer-dialog widget family, but several icon-only controls inside that family still need a focused pass before each draw proc can be named individually.
- `BuildingRecord.settlement_condition_percent` is behaviorally secure as a 0-100 settlement condition meter, but the original label could have been morale, approval, satisfaction, or prosperity rather than generic condition.
- `BuildingRecord.settlement_crisis_state` and `Building_UpdateSettlementCrisis` are secure as a countdown-driven population-loss/income-suppression state, but the exact original crisis label is still ambiguous.
